...
void TIM4_init(void) {
	...
	//DEBUG2(("命令模式启动\t\t完成\r\n"));
	DEBUG2(("Command Mode startup\t\tCompleted\r\n"));
}

/*
外部中断初始化，使用PB5，上升沿触发中断
PB0 -> PB5
*/
void EXTI_init(void) {
	NVIC_InitTypeDef NVIC_InitStructure;
	...
	//DEBUG2(("外部中断配置\t\t完成\r\n"));
	DEBUG2(("External Interrupt Config\t\tCompleted\r\n"));
}
...
void TIM3_init(void) {
	...
	//DEBUG2(("自动重发配置\t\t完成\r\n"));
	//DEBUG2(("发送监控配置\t\t完成\r\n"));
	DEBUG2(("Auto Retry Config\t\tCompleted\r\n"));
	DEBUG2(("Send Monitor Config\t\tCompleted\r\n"));
}
